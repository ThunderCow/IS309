...
This test script tests primarily whether package VOLUNTEER_PKG is functioning properly.

Only one of the tests is a test for an error condition.
...
/* TEST 3 
    PROCEDURE: VOLUNTEER3A.CREATE_PERSON_PP
    EXPECTED RESULT:    Success.  This procedure should return the same person ID 
    as the previous one because the data are a duplicate of the first one.
*/
...
/* TEST 4: 
    PROCEDURE: VOLUNTEER3A.CREATE_PERSON_PP
    EXPECTED RESULT:  This procedure should produce the error message 
        'Missing mandatory value for parameter (p_person_email) in context (CREATE_PERSON_SP).  No person added.'
        A NULL value for p_person_id should be returned to the calling environment.
*/
...
BEGIN
  P_PERSON_EMAIL := NULL;
  P_PERSON_GIVEN_NAME := 'Kevin';
  P_PERSON_SURNAME := 'Figglestein';
  P_PERSON_PHONE := '+1 8168892092';
